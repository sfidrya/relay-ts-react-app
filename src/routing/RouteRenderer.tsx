...
export default function RouterRenderer() {
  ...
  const [routeEntry, setRouteEntry] = useState<RouteEntry | undefined>(
    router?.get()
  )
  ...
  return (
    <ErrorBoundary>
      <Suspense fallback={'Loading fallback...'}>{routeComponent}</Suspense>
    </ErrorBoundary>
  )
}
...
function RouteComponent(props: RouteComponentProps) {
  ...
  console.log('COMPONENT FAILS: ', Component)
